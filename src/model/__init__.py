...
def _sanitize_attn_implementation(model_args):
    """Fall back to sdpa if flash_attention_2 is requested but not installed."""
    if model_args.get("attn_implementation", None) == "flash_attention_2":
        try:
            import flash_attn  # noqa: F401
        except ImportError:
            logger.warning("flash_attn is not installed; falling back to attn_implementation='sdpa'.")
...
def get_model(model_cfg: DictConfig):
    ...
    _sanitize_attn_implementation(model_args)
    ...
    # If model_path is a local directory, prevent huggingface_hub from trying
    # to validate it as a remote repo id (avoids HFValidationError with
    # trust_remote_code=True on newer huggingface_hub versions).
    ...
    try:
        ...
        error_msg = str(e)
        if "flash_attn" in error_msg.lower() or "flashattention" in error_msg.lower():
            logger.warning(f"flash_attn unavailable, retrying with attn_implementation='sdpa'. Original error: {e}")
            model_args["attn_implementation"] = "sdpa"
            model = model_cls.from_pretrained(
                pretrained_model_name_or_path=model_path,
                torch_dtype=torch_dtype,
