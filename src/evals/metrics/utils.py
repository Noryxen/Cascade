...
def evaluate_probability(model, batch):
    ...
    # NumPy does not support bfloat16, so cast to float32 before conversion.
    avg_losses = avg_losses.detach().to(torch.float32).cpu().numpy().tolist()
    normalized_probs = normalized_probs.detach().to(torch.float32).cpu().numpy().tolist()
